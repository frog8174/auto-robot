...
# Automatically move towards detected objects
AUTO_STEER_ENABLED = True
AUTO_BASE_SPEED = 0.3  # Initial small speed for auto-movement
...
try:
    ...
    print(f"Camera initialized: {CAM_WIDTH}x{CAM_HEIGHT} @ {CAM_FPS}fps")
except Exception as e:
    print(f"CRITICAL ERROR initializing camera: {e}")
    # We might choose to exit here or continue without camera
    # raise e 
...
def process_yolo_detection(frame):
    ...
    print(f"[YOLO Debug] Predictions shape after processing: {predictions.shape}")
    ...
    if best_obj:
        ...
        if AUTO_STEER_ENABLED:
            ...
            # 
            base_speed = AUTO_BASE_SPEED

            # 
            steer = max(-base_speed*1.5, min(base_speed*1.5, output))
        ...
        # 
        if AUTO_STEER_ENABLED and state["motion"] != "stop":
...
def cleanup():
    ...
    try:
        ...
        print(f"Error setting stop_event: {e}")
    ...
    # 釋放相機
    try:
        if 'camera' in globals() and camera is not None and camera.isOpened():
            camera.release()
            print("Camera released (cleanup).")
    except Exception as e:
...
if __name__ == "__main__":
    start_camera_thread()
    print("Starting Flask server at http://0.0.0.0:8000")
    # threaded=True 讓 video / API 可以同時服務多個連線
